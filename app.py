...
speech_region = os.getenv('SPEECH_REGION')  # e.g. westus2
speech_key = os.getenv('SPEECH_KEY')
speech_private_endpoint = os.getenv('SPEECH_PRIVATE_ENDPOINT')  # e.g. https://my-speech-service.cognitiveservices.azure.com/ (optional)  # noqa: E501
speech_resource_url = os.getenv('SPEECH_RESOURCE_URL')  # e.g. /subscriptions/6e83d8b7-00dd-4b0a-9e98-dab9f060418b/resourceGroups/my-rg/providers/Microsoft.CognitiveServices/accounts/my-speech (optional, only used for private endpoint)  # noqa: E501
user_assigned_managed_identity_client_id = os.getenv('USER_ASSIGNED_MANAGED_IDENTITY_CLIENT_ID')  # e.g. the client id of user assigned managed identity accociated to your app service (optional, only used for private endpoint and user assigned managed identity)  # noqa: E501
# OpenAI resource (required for chat scenario)
azure_openai_endpoint = os.getenv('AZURE_OPENAI_ENDPOINT')  # e.g. https://my-aoai.openai.azure.com/
azure_openai_api_key = os.getenv('AZURE_OPENAI_API_KEY')
azure_openai_deployment_name = os.getenv('AZURE_OPENAI_DEPLOYMENT_NAME')  # e.g. my-gpt-35-turbo-deployment
# Cognitive search resource (optional, only required for 'on your data' scenario)
cognitive_search_endpoint = os.getenv('COGNITIVE_SEARCH_ENDPOINT')  # e.g. https://my-cognitive-search.search.windows.net/
cognitive_search_api_key = os.getenv('COGNITIVE_SEARCH_API_KEY')
cognitive_search_index_name = os.getenv('COGNITIVE_SEARCH_INDEX_NAME')  # e.g. my-search-index
# Customized ICE server (optional, only required for customized ICE server)
ice_server_url = os.getenv('ICE_SERVER_URL')  # The ICE URL, e.g. turn:x.x.x.x:3478
ice_server_url_remote = os.getenv('ICE_SERVER_URL_REMOTE')  # The ICE URL for remote side, e.g. turn:x.x.x.x:3478. This is only required when the ICE address for remote side is different from local side.  # noqa: E501
ice_server_username = os.getenv('ICE_SERVER_USERNAME')  # The ICE username
ice_server_password = os.getenv('ICE_SERVER_PASSWORD')  # The ICE password
...
def handleUserQuery(user_query: str, client_id: uuid.UUID):
    ...
    # print(f"Chinmoy :: User: {user_query}")
    # print(f"Chinmoy :: Messages: {messages}" )
...
# Respond the user query and return the assistant reply. For chat scenario.
# The function is a generator, which yields the assistant reply in chunks.
def handleUserQuery1(user_query: str, client_id: uuid.UUID):
    client_context = client_contexts[client_id]
    messages = client_context['messages']
    data_sources = client_context['data_sources']
    ...
    # Call a API POST method with user_query as body
    response = requests.post(
        f"https://agentsapi.blueground-6d0da10d.eastus.azurecontainerapps.io/chat",
        headers={
    ...
    print(f"Chinmoy :: Response Status Code: {response.content}")
    responseText = json.loads(response.content)
    print(f"Chinmoy :: Response Text: {responseText['response']}")

    is_first_chunk = True
    is_first_sentence = True

    # NEW: iterate over the actual textual response instead of the raw response object
    full_response_str = responseText['response'] or ''
    for response_token in full_response_str:
        ...
        if response_token is not None:
            if is_first_chunk:
                ...
                yield f"<FTL>{first_token_latency_ms}</FTL>"
                is_first_chunk = False
            # Remove any OYD doc refs if a multi-char accumulation later (single char unlikely to match, kept for parity)
            if oyd_doc_regex.search(response_token):
                response_token = oyd_doc_regex.sub('', response_token).strip()
            yield response_token
            assistant_reply += response_token

            # Sentence / newline handling (accumulate spoken_sentence similar to streaming logic)
            if response_token == '\n':
                if is_first_sentence:
                    first_sentence_latency_ms = round((datetime.datetime.now(pytz.UTC) - aoai_start_time).total_seconds() * 1000)
                    print(f"AOAI first sentence latency: {first_sentence_latency_ms}ms")
                ...
                clean_token = response_token.replace('\n', '')
                spoken_sentence += clean_token
                # Punctuation boundary check (single char)
                if len(clean_token) == 1:
                    if clean_token in sentence_level_punctuations:
                        if is_first_sentence:
                            first_sentence_latency_ms = round((datetime.datetime.now(pytz.UTC) - aoai_start_time).total_seconds() * 1000)
                            print(f"AOAI first sentence latency: {first_sentence_latency_ms}ms")
                            yield f"<FSL>{first_sentence_latency_ms}</FSL>"
    ...
    # Flush any remaining accumulated sentence
    if spoken_sentence != '':
        speakWithQueue(spoken_sentence.strip(), 0, client_id)
        spoken_sentence = ''

...
// respond to Handle user query. Send user query to the Agentic API and display the response.
function handleUserQuery1(userQuery) {
    lastInteractionTime = new Date()
    chatRequestSentTime = new Date()

    // External REST API endpoint (replace later)
    const API_URL = 'https://agentsapi.blueground-6d0da10d.eastus.azurecontainerapps.io/chat'

    // (Optional) If you still want websocket path fallback, uncomment below:
    ...
    .then(response => {
        if (!response.ok) {
            throw new Error(`External API response status: ${response.status} ${response.statusText}`)
        }

        let chatHistoryTextArea = document.getElementById('chatHistory')
        chatHistoryTextArea.innerHTML += (chatHistoryTextArea.innerHTML.endsWith('\n\n') || chatHistoryTextArea.innerHTML === '' ? '' : '\n\n')
        chatHistoryTextArea.innerHTML += 'Assistant: '
        chatHistoryTextArea.scrollTop = chatHistoryTextArea.scrollHeight
        ...
        function read() {
            return reader.read().then(({ value, done }) => {
                ...
                // Optional STT latency tags if upstream includes them
                if (sttLatencyRegex.test(chunkString)) {
                    let sttLatency = parseInt(sttLatencyRegex.exec(chunkString)[0].replace('<STTL>', '').replace('</STTL>', ''))
                    console.log(`STT latency: ${sttLatency} ms`)
                    let latencyLogTextArea = document.getElementById('latencyLog')
    ...
    .catch(err => {
        console.error(err)
        let chatHistoryTextArea = document.getElementById('chatHistory')
        chatHistoryTextArea.innerHTML += `[Error: ${err.message}]`
        chatHistoryTextArea.scrollTop = chatHistoryTextArea.scrollHeight
    })
}
